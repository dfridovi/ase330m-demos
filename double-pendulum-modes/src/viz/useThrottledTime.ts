...
export function useThrottledTime(stepsPerSecond = 5): number {
  ...
  return usePendulumStore((s) => Math.round(s.currentTime / step) * step);
}
 ...
 * axis/grid/legend relayout per tick, not just the moving element) — see aircraft-explorer's
 * copy of this file for the profiling detail. The pendulum canvas itself reads currentTime
 * imperatively every animation frame and is unaffected; only these chart playhead markers
 * pause during active playback, resuming the moment playback is paused or scrubbed.
 */
export function useChartPlayheadTime(stepsPerSecond = 5): number {
  const step = 1 / stepsPerSecond;
  return usePendulumStore((s) => (s.isPlaying ? PLAYHEAD_FROZEN : Math.round(s.currentTime / step) * step));
}
