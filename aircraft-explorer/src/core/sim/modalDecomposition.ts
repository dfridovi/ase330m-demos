...
export function simulateModes(
  ...
  // Derived from B's own column count (not hardcoded) so this stays correct for both the
  // single-input longitudinal system and the two-input (aileron, rudder) lateral one.
  const inputCount = stateSpace.B[0]?.length ?? 1;
  const noInput = zeroInput(inputCount);
